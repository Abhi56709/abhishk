...
def findproduct(request):
    if request.method == 'POST':
        ...
        # print(x)
        mydata = Product.objects.filter(Q(product_name__icontains = x) | Q(product_name__icontains = x) | Q(product_id__icontains = x))
       # print(mydata)
        if mydata:
...
def contact(request):
    if request.method == 'GET':
        ...
        new_data.save()
        return render(request, 'test1/contact.html',{'x': 'Message sent succesfully'})
    ...
    if request.method == 'GET':
        ...
        if b==c:
            if (User.objects.filter(username=a)):
                return render(request, 'test1/signupuser.html', {'form':UserCreationForm(), 'error':'User name already exists try again with differnet user name'})
            else:
            ...
            return render(request, 'test1/signupuser.html', {'form':UserCreationForm(), 'error':'password Missmatch Try again'})
